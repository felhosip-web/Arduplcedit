...
export const StateMachineEditorModal: React.FC<StateMachineEditorModalProps> = ({
  ...
  const handleAddTransition = () => {
    ...
    let condition: StateMachineCondition = { kind: 'always' };
    if (newTransCond) {
      condition = {
        kind: 'comparison',
        left: newTransCond,
        operator: '==',
        right: 1 // Workaround for TS string | number limitation vs UI bool logic
      };
    }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4 border-r border-slate-800 pr-0 md:pr-6">
              ...
              <div className="space-y-2">
                {states.map(s => {
                  const isActive = simActiveStateId === s.id;
                  return (
                  <div key={s.id} className={`flex items-center justify-between p-2 rounded border ${isActive ? 'bg-emerald-900/50 border-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.3)]' : s.isInitial ? 'bg-indigo-900/30 border-indigo-700' : 'bg-slate-800 border-slate-700'}`}>
                    ...
                    <div key={t.id} className="bg-slate-800 p-2 rounded border border-slate-700 flex flex-col gap-2">
                      ...
                      {t.condition.kind !== 'always' && (
                        <div className="text-xs font-mono text-amber-400 bg-slate-950 px-2 py-1 rounded w-fit border border-amber-900/50">
                          IF {t.condition.kind === 'comparison' ? `${t.condition.left} ${t.condition.operator} ${t.condition.right}` : t.condition.kind}
                        </div>
                      )}
                    </div>
                  );
